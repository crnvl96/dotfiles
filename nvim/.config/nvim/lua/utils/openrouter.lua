...
local function filter_out_messages(message)
  ...
  for key, _ in pairs(message) do
    if not vim.tbl_contains(allowed, key) then message[key] = nil end
  end
  return message
end
  ...
  handlers = {
    setup = function(self)
      ...
      if model_opts and model_opts[model] and model_opts[model].opts then
        ...
        if not model_opts[model].opts.has_vision then self.opts.vision = false end
      end
    ...
    form_reasoning = function(_, data)
      local content = vim
        .iter(data)
        :map(function(item) return item.content end)
        :filter(function(content) return content ~= nil end)
        :join('')
    ...
    form_tools = function(self, tools)
      if not self.opts.tools or not tools then return end
      if vim.tbl_count(tools) == 0 then return end
    ...
    chat_output = function(self, data, tools)
      ...
      if self.temp.reasoning_effort then can_reason = true end
    ...
    inline_output = function(self, data)
      ...
      if data and data ~= '' then
        local ok, json =
          pcall(vim.json.decode, data.body, { luanil = { object = true } })
    ...
    on_exit = function(_, data)
      if data and data.status >= 400 then log:error('Error: %s', data.body) end
    end,
  },
  schema = {
    model = {
    ...
    reasoning_effort = {
      ...
      condition = function(self)
        ...
        if
          self.schema.model.choices[model] and self.schema.model.choices[model].opts
        then
